...
vector< float > get_svm_detector( const Ptr< SVM >& svm ) {
    // get the support vectors and stores it in the form of a Matrix of float, each support vector is a row
    Mat sv = svm->getSupportVectors();
    // Number of rows of the matrix (i.e., number of vectors in the SVM)
    const int sv_total = sv.rows;
    // get the decision function
    Mat alpha, svidx;
    double rho = svm->getDecisionFunction( 0, alpha, svidx );
    
    // Check the conditions of the SVM
...
void load_images( const String & dirname, vector< Mat > & img_lst, int width, int height, bool showImages = false, bool isNegative = false) {
    ...
    // Loop through all the filename, create the image, and put it into the image list
    for ( size_t i = 0; i < files.size(); ++i )
    {
        Mat imageRaw = imread( files[i] ); // load the image
        Mat img;
        
        if ( imageRaw.empty() )            // invalid image, skip it.
        {
...
// Compure the HOG descriptors for all image in the input image list, store the computed value in the gradient list
// To maximize the use of the image database, use_flip can be used so the total images created could be double
void computeHOGs( const Size wsize, const vector< Mat > & img_lst, vector< Mat > & gradient_lst, bool use_flip ) {
    // Create a hog descriptor with default params (check documentation)
    HOGDescriptor hog;
    // Set the winSize to the input winSize.
    // ATTENTION: In training, the winSize should be the image size
    hog.winSize = wsize;
    // Variable for gray image version of the current picture
    Mat gray;
    ...
    {
    	// Check to see if the image has the expected winSize
        if ( img_lst[i].cols >= wsize.width && img_lst[i].rows >= wsize.height )
        {
        	// Preparing the cropping rectangle if needed
            Rect r = Rect(( img_lst[i].cols - wsize.width ) / 2,
                          ( img_lst[i].rows - wsize.height ) / 2,
                          wsize.width,
                          wsize.height);
            // Convert the image (or cropped if needed) to gray scale
            cvtColor( img_lst[i](r), gray, COLOR_BGR2GRAY );
            
            // Compute hog descriptors
            hog.compute( gray, descriptors, Size( 8, 8 ), Size( 0, 0 ) );
            // Save the descriptors to the gradient lists
            gradient_lst.push_back( Mat( descriptors ).clone() );
            // Repeat the above steps with flipped version if needed
            if ( use_flip )
            {
                flip( gray, gray, 1 );
                hog.compute( gray, descriptors, Size( 8, 8 ), Size( 0, 0 ) );
...
int main( int argc, char** argv )
{
    ...
    // Lists containing all the image (positive, negative, and their gradient)
    vector< Mat > pos_lst, full_neg_lst, neg_lst, gradient_lst;
    vector< int > labels;
    clog << "Positive images are being loaded..." ;
    load_images( pos_dir, pos_lst, detector_width, detector_height, visualization);
    if ( pos_lst.size() > 0 )
    {
        clog << "...[done]" << endl;
    }
    ...
    else
    {
        for ( size_t i = 0; i < pos_lst.size(); ++i )
        {
            ...
            {
            	// TODO: Instead of cancel the image, just delete the corrupted image.
